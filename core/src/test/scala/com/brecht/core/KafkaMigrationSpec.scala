...
import com.brecht.cdc.PostgreSQLInstance
import com.brecht.cdc.scaladsl.ChangeDataCapture
import com.brecht.core.KafkaEventEnvelope._
import com.brecht.core.postgresql.PostgresSQLDataAccess
...
class KafkaMigrationSpecWithPostgreSQL1012 extends KafkaMigrationSpec {
  override def postgreSQLImageName =
    "brechtian-docker-images.bintray.io/brecht/postgresql:10.12"
}

class KafkaMigrationSpecWithPostgreSQL117 extends KafkaMigrationSpec {
  override def postgreSQLImageName =
    "brechtian-docker-images.bintray.io/brecht/postgresql:11.7"
}

class KafkaMigrationSpecWithPostgreSQL122 extends KafkaMigrationSpec {
  override def postgreSQLImageName =
    "brechtian-docker-images.bintray.io/brecht/postgresql:12.2"
}
...
abstract class KafkaMigrationSpec
  ...
  val brechtConfig = BrechtConfig(system.toTyped)
  ...
  override def beforeAll(): Unit = {
    ...
    val accountsTopicName = brechtConfig.getTopicName("test", "accounts")
    val megaCorpEventsTopicName = brechtConfig.getTopicNameForTag("test", "megacorp")
  ...
  test("Reading Kafka topics") {
    ...
    val accountsTopicName = brechtConfig.getTopicName("test", "accounts")
    val megaCorpTopicName = brechtConfig.getTopicNameForTag("test", "megacorp")
